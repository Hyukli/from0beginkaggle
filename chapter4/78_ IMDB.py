...
X_text=[]
for review in train['review']:
    X_text.append(' '.join(review_to_text(review,True)))
...
count_y_predict=gs_count.predict(X_text)
gs_tfidf=GridSearchCV(pip_tfidf,params_tfidf,cv=4,n_jobs=-1,verbose=1)
gs_tfidf.fit(X_train,y_train)
print gs_tfidf.best_score_
print gs_tfidf.best_params_

tfidf_y_predict=gs_tfidf.predict(X_text)
...
unlabled_train=pd.read_csv('../Datasets/IMDB/unlabeledTrainData.tsv',\
                           delimiter='\t',quoting=3)
...
def makeFeatureVec(words,model,num_features):
    featureVec=np.zero((num_features,),dtype="float32")
    nwords=0.
...
params_gbc={'n_estimator':[10,100,500],'learning_rate':[0.01,0.1,1.0],'max_depth':[2,3,4]}
